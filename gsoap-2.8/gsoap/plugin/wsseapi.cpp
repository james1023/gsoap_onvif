...
{ _wsse__Security *security = soap_wsse_add_Security(soap);
  ...
  */
  ...
  // james: 20140502 add BinarySecurityToken.
...
int av_soap_wsse_add_UsernameTokenDigest(struct soap *soap, const char *id, const char *username, const char *password, time_t diff)
{ 
    _wsse__Security *security = soap_wsse_add_Security(soap);
    time_t now = time(NULL);
    now -= diff;
...
{ _wsse__UsernameToken *token = soap_wsse_UsernameToken(soap, NULL);
    ...
    { /* check password digest: compute SHA1(created, nonce, password) */
      ...
      { char HA1[SOAP_SMD_SHA1_SIZE], HA2[SOAP_SMD_SHA1_SIZE];
        ...
        */
        ...
        { 
			// james: 110720 mark soap_wsse_session_verify for ARMLinux.
#if 0
  ...
  // james: 120224 mark soap_wsse_fault.
#if 1
...
// james: 121206 add av_soap_wsse_verify_Password.
int av_soap_wsse_verify_Password(struct soap *soap, const char *password)
{ _wsse__UsernameToken *token = soap_wsse_UsernameToken(soap, NULL);
  DBGFUN("soap_wsse_verify_Password");
  /* if we have a UsernameToken with a Password, check it */
  if (token && token->Password)
  { /* password digest or text? */
    ...
    { /* check password digest: compute SHA1(created, nonce, password) */
      ...
      { 
		...
		memset(HA1, 0, SOAP_SMD_SHA1_SIZE);
		memset(HA2, 0, SOAP_SMD_SHA1_SIZE);

        /* compute HA1 = SHA1(created, nonce, password) */
        calc_digest(soap, token->wsu__Created, nonce, noncelen, password, HA1);
		HA1[SOAP_SMD_SHA1_SIZE-1] = 0;
        /*
        calc_digest(soap, token->wsu__Created, token->Nonce, strlen(token->Nonce), password, HA1);
        */
        /* get HA2 = supplied digest from base64 Password */
        soap_base642s(soap, token->Password->__item, HA2, SOAP_SMD_SHA1_SIZE, NULL);
		HA2[SOAP_SMD_SHA1_SIZE-1] = 0;

		//DEBUG_TRACE(CHECK_ACTION_DBG, ("#%d CXMLHandler::soap_wsse_verify_Password, HA1=%s, len=%d, HA2=%s, len=%d, Password=%s. \n", __LINE__, HA1, strlen(HA1), HA2, strlen(HA2), token->Password->__item));

        /* compare HA1 to HA2 */
        if (!memcmp(HA1, HA2, SOAP_SMD_SHA1_SIZE))
        {
		  //DEBUG_TRACE(CHECK_ACTION_DBG, ("#%d CXMLHandler::soap_wsse_verify_Password, check1 ok. \n", __LINE__));
			// james: test
#if 0
		  // james: 110720 mark soap_wsse_session_verify for ARMLinux.
#ifdef _WIN32
...
// james: 121206 add av_soap_wsse_fault.
int av_soap_wsse_fault(struct soap *soap)
{
	return soap_wsse_fault(soap, wsse__FailedAuthentication, NULL);
}
