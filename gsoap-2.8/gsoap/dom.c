...
// james: 121220 add c style funciton.
void soap_dom_add_element(struct soap_dom_element *head, struct soap_dom_element *elt)
{
	struct soap_dom_element *e = NULL;

	elt->prnt = head;
	for (e=head->elts; e; e=e->next)
	{ 
		if (!e->next)
		{ 
			e->next = elt;
...
void soap_dom_add_attribute(struct soap_dom_element *elt, struct soap_dom_attribute *att)
{ 
	struct soap_dom_attribute *a = NULL;

	for (a = elt->atts; a; a = a->next)
	{ 
		if (!a->next)
		{ 
			a->next = att;
...
struct soap_dom_element *make_dom_element(struct soap *soap, const char *nstr, const char *name, const char *data)
{ 
	...
	if (nstr) elt->nstr = soap_strdup(soap, nstr);
	if (name) elt->name = soap_strdup(soap, name);
	if (data) elt->data = soap_strdup(soap, data);

	return elt;
}

struct soap_dom_attribute *make_dom_attribute(struct soap *soap, const char *nstr, const char *name, const char *data)
{ 
	struct soap_dom_attribute *att = (struct soap_dom_attribute  *)soap_malloc(soap, sizeof(struct soap_dom_attribute));
	soap_default_xsd__anyAttribute(soap, att);
	att->next = NULL;
	if (nstr) att->nstr = soap_strdup(soap, nstr);
	if (name) att->name = soap_strdup(soap, name);
	if (data) att->data = soap_strdup(soap, data);
	att->wide = NULL;
