...
{ 
	// james: 130118 modify soap_get0, soap_get1
#if 1
	if (soap->bufidx >= soap->buflen)
		return EOF;
...
{ /* Note: for MT systems, the main program MUST call soap_ssl_init() before any threads are started */
  ...
  // james: 130104 It has memory leak for SSL_library_init.
  if (!soap_ssl_init_done)
...
#define IP_MULTICAST_IF 2
  ...
  if (!soap_tag_cmp(endpoint, "https:*"))
  {
    ...
    { soap_mode m = soap->mode; /* preserve settings */
      ...
      soap->keep_alive = 1;
	  // james: test.
      if ((soap->fpost && soap->error = soap->fpost(soap, endpoint, host, port, NULL, NULL, 0))
...
{ register size_t count;
  ...
  // james: 100627 check http status.
  if (soap->httpcmd == 0)
	  return SOAP_OK;
...
# endif
  ...
  soap->gsoap_cb = NULL;
  soap->pPrivate = NULL;
  soap->gsoap_cmd_err = GSOAP_CMD_OK;
#endif
}
#endif
...
/*    const struct Namespace *ns1;
    ...
    // james: add one namespace.
    ns2_num = ns1_num+2;

    ns2_size = ns2_num*sizeof(struct Namespace);
    ns2 = (struct Namespace*)SOAP_MALLOC(soap, ns2_size);
    memset(ns2, 0, ns2_size);
    if (ns2)
    { 
        memcpy(ns2, soap->namespaces, ns1_num*sizeof(struct Namespace));
    }

    ns2[ns2_num-2].id = (char *)"tnjj";
    ns2[ns2_num-2].ns = "http://www.kimo.com.tw";
    soap->local_namespaces = ns2;*/
}
#endif
...
{ char host[sizeof(soap->host)];
  ...
  { unsigned int k = soap->mode;
    ...
    if ((k & SOAP_IO) != SOAP_IO_FLUSH)
      soap->mode |= SOAP_IO_BUFFER;
	// james: test.
    if ((soap->fpost && (soap->error=soap->fpost(soap, endpoint, soap->host, soap->port, soap->path, action, count))))
      return soap->error;
#ifndef WITH_LEANER
  ...
  { tSmbNtlmAuthRequest req;  
    ...
    { DBGLOG(TEST,SOAP_MESSAGE(fdebug, "NTLM S->C Type 1: received NTLM authentication challenge from server\n"));
      ...
      */
      ...
      soap->status = command;
	  // james: test.
      if ((soap->fpost && soap->fpost(soap, endpoint, host, port, soap->path, soap->action, 0))
       || soap_end_send_flush(soap))
        return soap->error;
      soap->mode = m;
      soap->keep_alive = k;
...
// james: 100701 add soap_sender_fault_subcode1.
int soap_sender_fault_subcode1(struct soap *soap, const char *faultsubcodeQName, const char *faultsubcodeQName1, const char *faultstring, const char *faultdetailXML)
{
	int ret = SOAP_OK;

	struct SOAP_ENV__Code *Subcode1 = NULL;

	if (soap->fault)
		return ret;

	ret = soap_sender_fault_subcode(soap, faultsubcodeQName, faultstring, faultdetailXML);
	if (soap->fault && soap->fault->SOAP_ENV__Code && soap->fault->SOAP_ENV__Code->SOAP_ENV__Subcode && 
		faultsubcodeQName1 && strlen(faultsubcodeQName1) > 0)
	{
		Subcode1 = 
		...
		memset(Subcode1, 0, sizeof(struct SOAP_ENV__Code));
		Subcode1->SOAP_ENV__Value = soap_strdup(soap, faultsubcodeQName1);
	}
...
// james: 100701 add soap_receiver_fault_subcode1.
int soap_receiver_fault_subcode1(struct soap *soap, const char *faultsubcodeQName, const char *faultsubcodeQName1, const char *faultstring, const char *faultdetailXML)
{
	int ret = SOAP_OK;

	struct SOAP_ENV__Code *Subcode1 = NULL;
	ret = soap_receiver_fault_subcode(soap, faultsubcodeQName, faultstring, faultdetailXML);
	if (soap->fault && soap->fault->SOAP_ENV__Code && soap->fault->SOAP_ENV__Code->SOAP_ENV__Subcode && 
		faultsubcodeQName1 && strlen(faultsubcodeQName1) > 0) 
	{
		Subcode1 = 
...
// james: 130201 add interface.
int soap_get_buflen(struct soap *soap)
{
	if (soap->buflen > 0) return soap->buflen;
	else return soap->bufidx; 
}

char *soap_get_buf(struct soap *soap)
{
	if (soap->buf) return soap->buf;
	return NULL;
}

void soap_put_buf_info(struct soap *soap, char *buf, unsigned int len)
{
	if (buf) {
		memcpy(soap->buf, buf, len);
		soap->buflen = len;
		soap->bufidx = 0;
	}
}

void av_soap_allocate(struct soap **soap, void *user_data)
{
	if (soap) {
		av_soap_deallocate(*soap);
		*soap = (struct soap *)malloc(sizeof(struct soap));
		// james: 130115 mark.
#if 1
		soap_init2(*soap, SOAP_IO_DEFAULT, SOAP_IO_DEFAULT);
#else
		soap_init2(*soap, SOAP_IO_DEFAULT|SOAP_XML_IGNORENS, SOAP_IO_DEFAULT|SOAP_XML_IGNORENS);
#endif
		(*soap)->gsoap_cb				= NULL;
		(*soap)->pPrivate				= user_data;
		// james: 130306 init.
#if 1
		(*soap)->chDescri[0] = 0x00;
		(*soap)->IsNoNeedRespond = 0;
		(*soap)->gsoap_cmd_err = GSOAP_CMD_OK;
		(*soap)->httpcmd = 0;
#endif
	}
...
void av_soap_deallocate(struct soap *soap)
{
	// james: 100609 *** free the collected memory.
	if (soap) {
		soap_end(soap);
		free(soap);
	}
